...
async function fetchNotUnpaidObjArray(){
    return new Promise(function(resolve, reject) {
        ...
        getDocs(notUnpaidQuery).then(snapshot => {
            ...
            let logData = []
            for (let elem in notUnpaidArry) {
                logData.push({id: notUnpaidArry[elem].name, status: ((notUnpaidArry[elem].status === "paid") ? ("complete") : (notUnpaidArry[elem].status)), total: notUnpaidArry[elem].amount})
            }
            resolve(logData)
        })
    })
